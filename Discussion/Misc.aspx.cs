...
public partial class Discussion_Misc : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack)
        {
            ...
            data = MyAdoHelper.ExecuteSelect("select * from posts where section='MISC.ASPX' and (content like '%" + search + "%' or title like '%" + search + "%')");
        ...
        {
            data = MyAdoHelper.ExecuteSelect("select * from posts where section='MISC.ASPX'");
            for (int i = 0; i < data.Rows.Count; i++)
            {
                html_string += "<a style=\"text-decoration:none;\" href=\"ViewPost.aspx?id=" + data.Rows[i]["id"] + "\"><div class=\"wall\">";
                html_string += "<div class=\"title\">" + data.Rows[i]["title"] + "</div>";
